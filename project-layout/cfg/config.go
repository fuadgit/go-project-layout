...
const defaultConfigFilePath = "app.json" // TODO Update config file name
...
// configData defines the ??? configuration file structure. // TODO Replace ??? by App name
...
// HomePage renders the ??? configuration.
func (c *Config) HomePage() string {
	return "<html>" +
		...
		render("comment", c.configData.Comment) +
		render("allow CORS", strconv.FormatBool(c.AllowCORS())) +
		render("server", c.Server().String()) +
		render("database", c.configData.Db.String()) +
...
// BasicAuthRealm provide the realm to be used for Basic Authentication.
func (c *Config) BasicAuthRealm() string {
	return c.configData.BasicAuthRealm
}
